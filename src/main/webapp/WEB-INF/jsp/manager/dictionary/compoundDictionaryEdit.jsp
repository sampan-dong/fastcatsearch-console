...
$(document).ready(function(){
	...
	searchInputObj.keydown(function (e) {
		if(e.keyCode == 13){
			...
			loadDictionaryTab("compound", '<%=dictionaryId %>', 1, keyword, searchColumnObj.val(), exactMatchObj.is(":checked"), true, '<%=targetId%>');
			return;
		}
	});
...
function <%=dictionaryId%>deleteOneWord(deleteId){
	if(confirm("Are you sure to delete?")){
		loadDictionaryTab("compound", '<%=dictionaryId %>', '${pageNo}', '${keyword}', searchColumnObj.val(), exactMatchObj.is(":checked"), true, '<%=targetId%>', deleteId);
	}
}
function <%=dictionaryId%>deleteSelectWord(){
	...
	loadDictionaryTab("compound", '<%=dictionaryId %>', '${pageNo}', '${keyword}', searchColumnObj.val(), exactMatchObj.is(":checked"), true, '<%=targetId%>', deleteIdList);
}
</script>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-footer">
				<form action="compound/upload.html" method="POST" enctype="multipart/form-data" style="display: inline;">
